...
namespace GatewayService
{
    class Proc_TEXOL_Data
    {
        ...
        public Proc_TEXOL_Data(cls_Gateway_Info Gateway, Put_ProcRecv_CollectData_Event _delegate_Method)
        {
            _Gateway = Gateway;
        ...
        public void ThreadPool_Proc(string msg)
        {
            try
            {
                ...
                if (dictObj.ContainsKey("PointID") == false)
                    return;
                else
                {
                    ...
                    cls_Device_Info Device = _Gateway.device_info.Where(p => p.device_name == dictObj["PointID"].ToString()).FirstOrDefault();
                    if (Device == null)
                        return;
                    else
                    {
            ...
            {
                throw new Exception(string.Format("Handle TEXOL Data Error Message Error [{0}].", ex.Message));
            }
        }
    }
}
